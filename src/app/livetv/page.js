...
export default function LiveTVPage() {
  ...
  useEffect(() => {
    fetchCountries();
    const savedCountry = localStorage.getItem('luxa_iptv_country');
    if (savedCountry) {
  ...
  const fetchChannels = async (countryCode) => {
    try {
      ...
      localStorage.setItem('luxa_iptv_country', countryCode);
  ...
  const resetSelection = () => {
    ...
    setChannels([]);
    setSearchQuery('');
    localStorage.removeItem('luxa_iptv_country');
  };
              ...
              <div className="channel-grid">
                {channels.map((channel) => (
                  <motion.div
                    ...
                    key={channel.id}
                  >
                    <Link href={`/watch/iptv?id=${channel.id}`} className="channel-card glass-card">
