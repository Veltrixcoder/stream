...
export default function LibraryPage() {
  ...
  useEffect(() => {
    const localHistory = JSON.parse(localStorage.getItem('luxa_history') || '[]');
    const localSaved = JSON.parse(localStorage.getItem('luxa_saved') || '[]');
    setHistory(localHistory);
    setSaved(localSaved);
  }, []);

  const clearHistory = () => {
    localStorage.removeItem('luxa_history');
    setHistory([]);
  };

  const removeItem = (id, type) => {
    const key = type === 'history' ? 'luxa_history' : 'luxa_saved';
